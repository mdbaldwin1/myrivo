...
import { buildStorefrontPoliciesPath } from "@/lib/storefront/paths";
...
export default async function PoliciesPage({ searchParams }: PoliciesPageProps) {
  ...
  if (requestedStoreSlug && !disableStoreCanonicalRedirect) {
    redirect(`${buildStorefrontPoliciesPath(requestedStoreSlug)}${buildSearchSuffix(resolvedSearchParams, ["store"])}`);
  }
