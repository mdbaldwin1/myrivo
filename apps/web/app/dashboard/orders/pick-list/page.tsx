...
export default async function DailyPickListPage() {
        ...
        <ul className="space-y-2 text-sm">
          {(orders ?? []).map((order) => (
            <li key={order.id} className="rounded border p-3">
              ...
              <Link className="text-primary underline" href={`/dashboard/orders/${order.id}/packing-slip`} target="_blank" rel="noreferrer">
                Print packing slip
              </Link>
            </li>
          ))}
