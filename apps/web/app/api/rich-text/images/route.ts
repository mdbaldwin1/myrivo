...
function sanitizeFolder(input: string) {
  return input.toLowerCase().replace(/[^a-z0-9/_-]/g, "") || "rich-text";
}
...
export async function POST(request: NextRequest) {
  ...
  const storeSlug = String(formData.get("storeSlug") ?? request.nextUrl.searchParams.get("storeSlug") ?? "").trim();
  const folder = sanitizeFolder(String(formData.get("folder") ?? "rich-text").trim());
  ...
  const path = `${bundle.store.id}/${folder}/${Date.now()}-${randomUUID()}.${ext}`;
