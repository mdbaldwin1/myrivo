...
import type { ResolvedStoreLegalDocument } from "@/lib/legal/store-documents";

type StoreLegalDocumentPreviewDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  title: string;
  description: string;
  storeName: string;
  document: ResolvedStoreLegalDocument;
};

export function StoreLegalDocumentPreviewDialog({
  open,
  onOpenChange,
  title,
  description,
  storeName,
  document
}: StoreLegalDocumentPreviewDialogProps) {
  return (
          ...
          <div className="h-full overflow-y-auto p-6 sm:p-8">
            <div className="mx-auto max-w-4xl space-y-6">
              <div className="space-y-2 border-b border-border/60 pb-5">
                <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">{storeName}</p>
                <h2 className="text-3xl font-semibold [font-family:var(--storefront-font-heading)]">{document.title}</h2>
