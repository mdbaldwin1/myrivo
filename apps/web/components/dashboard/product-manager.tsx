...
const variantStatusOptions: Array<ProductVariantListItem["status"]> = ["active", "archived"];

function createBlankVariant(isDefault = false): VariantDraft {
  return {
    id: undefined,
    title: "",
    sku: "",
...
function resolvePriceRange(variants: ProductVariantListItem[]) {
  if (variants.length === 0) {
    return "$0.00";
  }
...
function hasStructuredVariants(product: ProductListItem) {
  const variants = product.product_variants ?? [];
  return variants.length > 1 || variants.some((variant) => Object.keys(variant.option_values ?? {}).length > 0);
}

function resolveTierNamesForProduct(product: ProductListItem) {
