...
describe("guest digital access recovery route", () => {
  test("returns the exact same neutral response and padded duration for a valid and invalid pair", async () => {
    const { createDigitalLinkRequestHandler } = await import(
      "@/lib/digital-products/customer-recovery-handler"
    );
    const valid = handlerDependencies({ queued: true, workMs: 40 });
    const invalid = handlerDependencies({ queued: false, workMs: 5 });
    ...
    expect(valid.elapsed()).toBe(350);
    expect(invalid.elapsed()).toBe(350);
    expect(valid.observedSleeps).toEqual([310]);
    expect(invalid.observedSleeps).toEqual([345]);
