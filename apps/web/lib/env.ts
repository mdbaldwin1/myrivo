...
export const serverEnvSchema = z.object({
  ...
  DIGITAL_RECOVERY_TRUSTED_IP_HEADER: z
    .string()
    .trim()
    .regex(/^[a-z0-9-]+$/i)
    .optional(),
...
export function getServerEnv() {
  if (!cachedServerEnv) {
    cachedServerEnv = serverEnvSchema.parse({
      ...
      DIGITAL_DELIVERY_PROCESS_SECRET: process.env.DIGITAL_DELIVERY_PROCESS_SECRET,
      DIGITAL_DELIVERY_TOKEN_SECRET: process.env.DIGITAL_DELIVERY_TOKEN_SECRET,
      DIGITAL_DOWNLOAD_SESSION_SECRET: process.env.DIGITAL_DOWNLOAD_SESSION_SECRET,
      DIGITAL_RECOVERY_TRUSTED_IP_HEADER: process.env.DIGITAL_RECOVERY_TRUSTED_IP_HEADER,
