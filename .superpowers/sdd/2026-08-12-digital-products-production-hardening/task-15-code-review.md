...
**FAIL** — the three deterministic correlation defects are fixed and a twelve-scenario fixture exists, but the tested shared verifier is still weaker than the actual CLI path, so the claimed valid full artifact does not satisfy the real release command.

### P1 — The full fixture passes the tested verifier but fails the real CLI's grant checks

**Evidence:** The twelve-scenario fixture's `five-grants` observation contains only five `issued` grant rows. Its provider artifact claims a `releasedFaultGrantId` generated by `nextId()`, but no observed `released` row has that ID (`apps/web/tests/fixtures/digital-acceptance-evidence.ts:32-39,72`). `verifyDigitalAcceptanceArtifact()` delegates to `verifyDigitalAcceptanceEvidence()`, whose grant branch checks only exact issued IDs, session-hash cardinality, and asset versions (`apps/web/lib/digital-products/acceptance-evidence.ts:134-139`). Therefore the new CLI-core test passes this artifact (`apps/web/tests/digital-acceptance-cli.test.ts:17-18`).

The actual release script continues after `verifyDigitalAcceptanceArtifact()` with additional manual logic that requires exactly one observed released row matching `releasedFaultGrantId`, with `released_at` and `last_safe_error`, plus a matching issued retry (`scripts/verify-digital-products-acceptance.mjs`, five-grants branch retained from the prior revision). The supposedly valid fixture would fail that real path.

**Impact:** Tests do not prove that any full artifact can pass the actual promotion command. Shared verification and CLI behavior remain divergent—the exact class of drift this work intended to eliminate.

**Remediation:** Move every remaining manual semantic predicate from the CLI into `verifyDigitalAcceptanceEvidence()`, including released-fault/retry, sixth denial, replacement, and any remaining provider correlations. Make the CLI a thin signature/target wrapper with no duplicate scenario semantics. Update the full fixture with one realistic released fault row and safe metadata, then test the same exported function the CLI uses as its complete semantic gate.

### P1 — Delivery resend evidence is still not correlated to persisted notification state

**Evidence:** `deliveryEvidence` includes `resendMessageId`, but the shared delivery verifier checks only job/attempt chronology (`apps/web/lib/digital-products/acceptance-evidence.ts:149-154`). The twelve-scenario delivery observation has no notifications, while its artifact claims `resendMessageId: "email_retry"` (`apps/web/tests/fixtures/digital-acceptance-evidence.ts:65-75`). It still passes the full fixture test. No adversarial case mutates the delivery resend ID.

**Impact:** A failed delivery can be marked retried/succeeded without proving the promised replacement email was durably sent by Resend. This leaves a core delivery-recovery release scenario self-attested.

**Remediation:** Add a succeeded Resend notification with persisted `provider_message_id` to the delivery observation and require exact equality with `resendMessageId`, appropriate notification type, sent timestamp, and provider. Add an adversarial mismatch case.

### P2 — Adversarial coverage is meaningful but not complete

The new mutation matrix covers replacement manifests/new order, delivery ownership/time, dispute mapping/access, Resend ID, refund signature, grants, and composition. It does not cover missing/duplicate required scenarios through the semantic verifier, released-fault/retry mismatch, delivery resend mismatch, refund amount/ID/source event mismatch, dispute charge/payment/event mismatch, freshness future chronology, or secret-leak behavior through the artifact verifier. Expand the table when consolidating CLI semantics.

### Previous findings disposition

- **Replacement old/new observation:** Resolved. The artifact carries a separately parsed new-order observation and verifier checks old manifest retention plus new checkout snapshot.
- **Delivery start/finish timestamps:** Resolved with explicit fields and exact observed chronology comparison.
