with seed_docs as (
  select
    'platform_terms'::text as key,
    'Myrivo Terms and Conditions'::text as title,
    'Platform terms for Myrivo accounts, workspaces, and shared platform use.'::text as description,
    'all'::text as audience,
    true as is_required,
    'seed-platform-terms-v1-baseline'::text as content_hash,
    'Initial thorough platform legal baseline'::text as change_summary,
    $$# Myrivo Terms and Conditions
...
Questions about these terms may be sent to legal@myrivo.com.$$::text as content_markdown
  union all
  select
    'platform_privacy',
    'Myrivo Privacy Policy',
    'Platform privacy and data handling policy for Myrivo accounts and services.',
    'all',
    true,
    'seed-platform-privacy-v1-baseline',
    'Initial thorough platform legal baseline',
    $$# Myrivo Privacy Policy
...
- send transactional messages, product or service notifications, and legally required notices
...
Questions about this Privacy Policy or Myrivo privacy practices may be sent to privacy@myrivo.com.$$::text
  union all
  select
    'store_privacy_base',
    ...
    'customer',
    false,
    'seed-store-privacy-base-v1-baseline',
    'Initial thorough storefront legal baseline',
    $$# Privacy Policy
...
Questions about this Privacy Policy or the store's privacy practices may be sent to {privacyContactEmail}.$$::text
  union all
  select
    'store_terms_base',
    ...
    'customer',
    false,
    'seed-store-terms-base-v1-baseline',
    'Initial thorough storefront legal baseline',
    $$# Terms & Conditions
...
If you have questions about these terms, contact us at {termsContactEmail}.$$::text
),
upserted_documents as (
  insert into public.legal_documents (key, title, description, audience, is_active)
  select key, title, description, audience, true
  from seed_docs
...
existing_documents as (
  select id, key
  from public.legal_documents
  where key in (select key from seed_docs)
),
target_documents as (
  select id, key from upserted_documents
  union
  select id, key from existing_documents
),
updated_versions as (
  ...
  set
    ...
    effective_at = coalesce(v.effective_at, now()),
    published_at = coalesce(v.published_at, now()),
    content_markdown = seed.content_markdown,
    content_hash = seed.content_hash,
    change_summary = seed.change_summary
  from seed_docs seed
  join target_documents d on d.key = seed.key
  where v.legal_document_id = d.id
    and v.version_label = 'v1.0'
    and (
      v.content_hash in ('seed-terms-v1', 'seed-privacy-v1', 'seed-store-privacy-base-v1', 'seed-store-terms-base-v1')
      or char_length(coalesce(v.content_markdown, '')) < 1200
    )
  returning v.id
),
...
latest_base_versions as (
  ...
  order by d.key, coalesce(v.published_at, v.effective_at, v.created_at) desc, v.created_at desc
),
store_defaults as (
  select
    d.id as store_legal_document_id,
    d.store_id,
    d.key,
    s.name as store_name,
    s.slug as store_slug,
    coalesce(nullif(ss.support_email, ''), 'support@example.com') as support_email,
    coalesce(
      nullif(d.published_variables_json ->> 'privacyContactEmail', ''),
      nullif(d.variables_json ->> 'privacyContactEmail', ''),
      nullif(ss.support_email, ''),
      'support@example.com'
    ) as privacy_contact_email,
    coalesce(
      nullif(d.published_variables_json ->> 'termsContactEmail', ''),
      nullif(d.variables_json ->> 'termsContactEmail', ''),
      nullif(ss.support_email, ''),
      'support@example.com'
    ) as terms_contact_email,
    coalesce(
      nullif(d.published_variables_json ->> 'governingLawRegion', ''),
      nullif(d.variables_json ->> 'governingLawRegion', ''),
      'the jurisdiction where the store operates'
    ) as governing_law_region
  from public.store_legal_documents d
  join public.stores s on s.id = d.store_id
  left join public.store_settings ss on ss.store_id = d.store_id
),
backfilled_store_docs as (
  update public.store_legal_documents d
  set
    source_mode = 'template',
    template_version = coalesce(nullif(d.template_version, ''), 'v1'),
    variables_json =
      jsonb_build_object(
        'storeName', defaults.store_name,
        'storeSlug', defaults.store_slug,
        'supportEmail', defaults.support_email,
        'privacyContactEmail', defaults.privacy_contact_email,
        'termsContactEmail', defaults.terms_contact_email,
        'governingLawRegion', defaults.governing_law_region
      ) || coalesce(d.variables_json, '{}'::jsonb),
    addendum_markdown = coalesce(d.addendum_markdown, ''),
    published_source_mode = 'template',
    published_template_version = base.version_label,
    published_title = case when d.key = 'privacy' then 'Privacy Policy' else 'Terms & Conditions' end,
    published_body_markdown = case
      when d.key = 'privacy' then
        replace(
          replace(
            replace(
              replace(base.content_markdown, '{storeName}', defaults.store_name),
              '{storeSlug}', defaults.store_slug
            ),
            '{supportEmail}', defaults.support_email
          ),
    ...
    published_variables_json =
      jsonb_build_object(
        ...
        'termsContactEmail', defaults.terms_contact_email,
        'governingLawRegion', defaults.governing_law_region
      ) || coalesce(d.variables_json, '{}'::jsonb) || coalesce(d.published_variables_json, '{}'::jsonb),
    published_addendum_markdown = coalesce(nullif(d.published_addendum_markdown, ''), coalesce(d.addendum_markdown, '')),
    published_base_document_version_id = base.id,
    published_base_version_label = base.version_label,
    published_version = greatest(coalesce(d.published_version, 1), 1),
    published_change_summary = coalesce(nullif(trim(d.published_change_summary), ''), 'Initial seeded storefront legal baseline'),
    effective_at = coalesce(d.effective_at, d.published_at, d.created_at, now()),
    published_at = coalesce(d.published_at, d.created_at, now())
  from store_defaults defaults
  join latest_base_versions base
    on (
      (defaults.key = 'privacy' and base.key = 'store_privacy_base')
      or (defaults.key = 'terms' and base.key = 'store_terms_base')
    )
  where d.id = defaults.store_legal_document_id
    and (
      coalesce(trim(d.published_body_markdown), '') = ''
      or d.published_base_document_version_id is null
      or d.published_base_version_label is null
      or coalesce(d.published_title, '') in ('Storefront Privacy Policy Base', 'Storefront Terms & Conditions Base', 'Storefront Terms Base')
    )
  returning d.id
)
...
select
  ...
  coalesce(d.published_title, case when d.key = 'privacy' then 'Privacy Policy' else 'Terms & Conditions' end),
  d.published_body_markdown,
  coalesce(d.published_variables_json, '{}'::jsonb),
  coalesce(d.published_addendum_markdown, ''),
  d.published_base_document_version_id,
  d.published_base_version_label,
  coalesce(d.published_change_summary, 'Initial seeded storefront legal baseline'),
  d.effective_at,
  d.published_at,
  d.published_by_user_id
from public.store_legal_documents d
where coalesce(trim(d.published_body_markdown), '') <> ''
  and not exists (
    select 1
