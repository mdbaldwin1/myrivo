-- Publishing readiness follows fulfillment down to the variant.
--
-- Two things were wrong once a variant could be delivered differently from its
-- product. A product that ships by default but sells one download skipped these
-- checks entirely, so it could go live with no file, no rights, and no buyer
-- preview. And a product that defaults to download demanded a file from every
-- active variant, so a painting offering a print or the original canvas could
-- never be published at all.
--
-- The rules now apply when any active variant is a download, and ask for files
-- only from those variants.
...
begin
  ...
  loop
    ...
    -- Fulfillment belongs to the variant, so a product is subject to these
    -- rules when any active variant is delivered as a download - not when the
    -- product as a whole happens to default to one.
    if not exists (
      select 1
      ...
      where variant.product_id = v_target.product_id
        ...
        and coalesce(variant.fulfillment_type, v_product.product_type) = 'digital'
    ) and not (
      v_product.product_type = 'digital'
      and not exists (
        select 1
        from public.product_variants variant
        where variant.product_id = v_target.product_id
